...
> **Project note (still-young-days):** This app uses no state-management package — dependencies flow through `AppDeps` (InheritedWidget). Do not flag the absence of BLoC/Riverpod. Accessibility is stricter than the generic checklist: contrast must be **>= 7:1** (senior users), sizes must come from `lib/theme/tokens.dart`, and time must be injected via `AppDeps.clock`. See `CLAUDE.md` for the full project rules.
